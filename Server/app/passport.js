            ...
            connection.query("SELECT * FROM Usuario WHERE mail = ?",[username], function(err, rows) {
                ...
                if (rows.length) {
                    ...
                    var newUserMysql = {
                        ...
                        type: "Al" // use the generateHash function in our user model
                    };

                    var insertQuery = "INSERT INTO Usuario ( mail, clave , nom, apell, grupo, tipo) values (?,?,?,?,?,?)";

                    connection.query(insertQuery,[newUserMysql.username, newUserMysql.password, newUserMysql.name, 
                        newUserMysql.lastname, newUserMysql.group, newUserMysql.type] ,
                        function(err, rows) {
                        if (err) throw err;
